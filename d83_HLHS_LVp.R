#This codes are used for compare Control with HLHS left ventricle CDH5+ cells
library(Seurat)
library(dplyr)
setwd("D:/SharedFolder/project/Mingxia/Seurat")
ind="LVp"
load(paste0("E:/Mingxia/v2/Seurat/d83_LVp/d83_LVp.Robj"))
d83=myData
load("LVp/LVp.Robj")
...
pdf("d83_HLHS_LVp/variable.genes.pdf")
...
pdf("d83_HLHS_LVp/PCA.heatmap.pdf")
...
pdf("d83_HLHS_LVp/determine.NO.PCs.pdf")
...
pdf("d83_HLHS_LVp/tSNE.pdf")
DimPlot(all, reduction.use = "tsne", pt.size =0.6,do.label=T)
DimPlot(all, reduction.use = "tsne", pt.size =0.6, group.by = "orig.ident")
dev.off()

pdf("d83_HLHS_LVp/uMAP.pdf")
DimPlot(all, reduction.use = "umap", pt.size =0.6,do.label=T)
p=DimPlot(all, reduction.use = "umap", pt.size =0.6, group.by = "orig.ident",do.return =T)
dev.off()

library(plotrix)
dat=cbind(all@meta.data,all@dr$umap@cell.embeddings)
gg_color_hue <- function(n) {
  hues = seq(15, 375, length = n + 1)
  hcl(h = hues, l = 65, c = 100)[1:n]
}
cols = gg_color_hue(2)
mycols=rep(cols[1],nrow(dat))
mycols[which(dat$orig.ident=="LVp")]=cols[2]
ygap <- ifelse(dat$UMAP2 < (-12), dat$UMAP2+40, dat$UMAP2)

pdf("d83_HLHS_LVp/uMAP.adjustY.pdf")
plot(dat$UMAP1, ygap, col=mycols, pch=16, xlab= 'UMAP_1', ylab='UMAP_2', yaxt="n",cex=0.4)
yat <- pretty(ygap)
yat <- yat[yat!=-12]
ylab <- ifelse(yat<(-12), yat-40, yat)
axis(2,at=yat, labels=ylab)
axis.break(2,-12,style="slash") 
dev.off()
...
write.table(combined.markers,"d83_HLHS_LVp/Clusters.DEGs.txt",quote=F,sep="\t")
top10 <- combined.markers %>% group_by(cluster) %>% top_n(10, avg_logFC)
pdf("d83_HLHS_LVp/Clusters.DEG.heatmap.pdf",height=12,width=9)
...
for(gene in markers){
  ...
  ggsave(p,filename = paste0("d83_HLHS_LVp/uMAP/",gene,".pdf"),height=7.5,width=6)
  ...
  ggsave(p,filename = paste0("d83_HLHS_LVp/violin/",gene,".pdf"),height=6,width=12)
}
...
for(cluster in names(table(all@meta.data$cluster))){
  if(cluster!=16){
    print(cluster)
    tmp <- FindMarkers(object = all, ident.1 = paste0("d83_LVp_",cluster), ident.2 = paste0("LVp_",cluster))
    gene_name=row.names(tmp)
    tmp=cbind(tmp,gene_name,cluster)
    results=rbind(results,tmp)
  }
}
write.table(results,"d83_HLHS_LVp/d83_vs_HLHS.clusters.DEGs.txt",quote=F,sep="\t")

write.table(table(all@ident),"d83_HLHS_LVp/cell.numers.txt",quote=F,sep="\t")
save(all,file="d83_HLHS_LVp/all.Robj")
...
pdf("d83_HLHS_LVp/d83_HLHS_LVp.Endocarduim.FC.barplot.pdf",height=9,width=6)
...
degs=degs[which(degs$p_val_adj<0.05),]
degs=degs[which(degs$gene_name %in% genes),]
degs=degs[which(degs$cluster %in% c(0,2,3,4,6,7,12,13,17)),]
degs$avg_logFC=-degs$avg_logFC
degs$absFC=abs(degs$avg_logFC)
degs1=degs %>% group_by(gene_name) %>% top_n(1,absFC)
degs1=degs1[order(degs1$avg_logFC,decreasing = TRUE),]
pdf("d83_HLHS_LVp/d83_HLHS_LVp.Endothelium.FC.barplot.pdf",height=6.5,width=6)
ggplot(degs1,aes(x=gene_name,y=avg_logFC))+geom_col(fill="orange",width=0.6)+geom_hline(yintercept = 0)+scale_x_discrete(limits=degs1$gene_name)+
  xlab("")+ylab(bquote('log'['2']*'Fold change'))+expand_limits(y=c(-3,2))+coord_flip()+
  theme(axis.title.x  = element_text( vjust=0.5, size=24,face="bold"),
        axis.text.x  = element_text( vjust=0.5, size=24,face="bold"),
        axis.text.y  = element_text( vjust=0.5, size=24,face="bold.italic"))
dev.off()

umap2=all@dr$umap@cell.embeddings[,2]
all@dr$umap@cell.embeddings[which(umap2<(-12)),2]=all@dr$umap@cell.embeddings[which(umap2<(-12)),2]+40
for(gene in c("CDH5","NPR3", "HAPLN1", "CDH11", "MGLL", "APLN", "APLNR")){
  print(gene)
  p=FeaturePlot(object = all, features = c(gene), reduction = "umap",cols.use=c("lightgrey", "blue"),pt.size=0.5, do.return =TRUE)
  ggsave(p[[1]],filename = paste0("d83_HLHS_LVp/uMAP/20190911/",gene,".pdf"),width=7,height = 7)
}
